...
export class ListaUbicaciones_Mapa1 {
    ...
    initGMaps(element: any) {
        ...
        this.fotosservice.getfotosserviciosMaxIdPromotor().subscribe((gfotos: any[]) => {
            //console.log("lista de ubicaciones 1: ", gfotos.length);
            for (var i = 0; i < gfotos.length; i++) {
                var arr = ["[id: " + gfotos[i].idpromotor + ", usuario: " + gfotos[i].idusuario + ", Hora: " + gfotos[i].FechaHora + "]", gfotos[i].latitud, gfotos[i].longitud, ''];
                this.rutas.push(arr);
            }
    ...
    initMaps(element: any) {
        ...
        for (i = 0; i < marker.length; i++) {
            ...
            //console.log("lista de ubicaciones 1: ", marker[i].title + " ; " + marker[i].color);
