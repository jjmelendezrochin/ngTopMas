...
export class ListaUbicaciones_Mapa {
    ...
    initGMaps(element: any) {
        ...
        this.fotosservice.getfotosservicios().subscribe((gfotos: any[]) => {
            //console.log("lista de ubicaciones: ", gfotos.length);
            for (var i = 0; i < gfotos.length; i++) {
                var arr = ["[id: " + gfotos[i].idpromotor + ", usuario: " + gfotos[i].idusuario + "]", gfotos[i].latitud, gfotos[i].longitud, ''];
                this.rutas.push(arr);
            }
